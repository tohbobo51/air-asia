...
{
    new Query[2560];
    mysql_format(voiceData, Query, sizeof(Query), "UPDATE `voicedata` SET `pUsername`='%e',`pTogRadio`='%d',`pTogMic`='%d',`pFreqRadio`='%d',`pSfxTurnOn`='%d',`pSfxTurnOff`='%d' WHERE `pID`='%d'",
...
stock SfxSoundTurnOn(freq)
{
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(IsPlayerConnected(i))
        {
            ...
            {
                PlayAudioStreamForPlayer(i, "http://play.happinesscity.tk/micon.mp3");
            }
        }
    }
}
...
stock SfxSoundTurnOff(freq)
{
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(IsPlayerConnected(i))
        {
            ...
            {
                PlayAudioStreamForPlayer(i, "http://play.happinesscity.tk/micoff.mp3");
            }
        }
    }
}
