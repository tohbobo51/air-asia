...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(strcmp(bData[id][bOwner], "-") || bData[id][bOwnerID] != 0)
		{
			...
			bData[id][bPickup] = CreateDynamicPickup(19198, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 25.0);
			bData[id][bPickup2] = CreateDynamicPickup(19198, 23, bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ]+0.2, id, bData[id][bInt], _, 25.0);
        }
        else
        {
