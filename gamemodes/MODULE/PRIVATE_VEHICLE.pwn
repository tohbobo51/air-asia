...
{
	if(pvData[i][cClaim] == 0 && pvData[i][cStolen] == 0 && pvData[i][cPark] < 0)
	{
...
function OnVehStarterpack(playerid, pid, model, color1, color2, Float:x, Float:y, Float:z, Float:a)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Kamu telah mengambil Startepack kota, 1 Faggio & Rp.2,500,000");
	return 1;
}
...
function OnVehRentPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Anda telah menyewa kendaraan seharga Rp.50.000 dengan model %s(%d)", GetVehicleModelName(model), model);
	new str[150];
	format(str,sizeof(str),"[VEH]: %s menyewa kendaraan seharga Rp.50.000 model %s(%d)!", GetRPName(playerid), GetVehicleModelName(model), model);
	LogServer("Property", str);
	pData[playerid][pBuyPvModel] = 0;
	return 1;
}
...
function OnVehRentBike(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Kamu telah menyewa kendaraan seharga Rp.50.000 dengan model %s(%d)", FormatMoney(GetVehicleRentalCost(model)), GetVehicleModelName(model), model);
	pData[playerid][pBuyPvModel] = 0;
	new str[150];
	format(str,sizeof(str),"[VEH]: %s menyewa kendaraan seharga Rp.50.000 model %s(%d)!", GetRPName(playerid), FormatMoney(GetVehicleCost(model)), GetVehicleModelName(model), model);
	LogServer("Property", str);
	return 1;
}
...
CMD:dandastreet(playerid, params[])
{
...
CMD:vehmenu(playerid, params[])
{
	...
	if((carid = Vehicle_Nearest(playerid)) != -1)
	{
		...
		{
    		ShowPlayerDialog(playerid, DIALOG_VRM, DIALOG_STYLE_LIST, "Asia Pride - Menu Kendaraan", "Mesin\nKunci\nLampu\nBagasi\nBuka/Tutup Hood\nBuka/Tutup Trunk", "Pilih", "Batal");
		}
...
/*CMD:lock(playerid, params[])
{
	    ...
	    if((carid = Vehicle_Nearest(playerid)) != -1)
	    {
			if(Vehicle_IsOwner(playerid, carid))
			{
				...
				else
				{
					...
					//SwitchVehicleAlarm(pvData[carid][cVeh], true);
					//SetTimerEx("AlarmOff", 1000, false, "d", pvData[carid][cVeh]);
	...
	else
	{
		...
		if(found)
			ShowPlayerDialog(playerid, DIALOG_LOCKVEH, DIALOG_STYLE_TABLIST_HEADERS, "Vehicle Lock", msg2, "Select", "Close");
		else
...
CMD:akseskendaraan(playerid, params[])
{
	new bool:found = false, msg2[512];
	format(msg2, sizeof(msg2), "Model\tStatus\n");
	foreach(new i : PVehicles)
...
{		
	...
	{
		new vehid;
		if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1325.27, -881.80, 39.57) && !IsPlayerInRangeOfPoint(playerid, 3.0, 1689.7552, -2311.7261, 13.5469) && !IsPlayerInRangeOfPoint(playerid, 3.0, 804.9299, -1362.8210, 13.5469) && !IsPlayerInRangeOfPoint(playerid, 3.0, 1757.7032, -1864.0552, 13.5743)) return Error(playerid, "You must in showroom/dealer!");
		if(sscanf(params, "d", vehid)) return Usage(playerid, "/unrentpv [vehid] | /myv - for find vehid");
		if(vehid == INVALID_VEHICLE_ID) return Error(playerid, "Invalid id");
		...
		foreach(new i : PVehicles)			
		{
			if(vehid == pvData[i][cVeh])
			{
				...
				else return Error(playerid, "ID kendaraan ini bukan punya mu! gunakan /v my(/myv) untuk mencari ID.");
			}
		}
		return 1;
	}	
...
CMD:kasihkendaraan(playerid, params[])
{
	new vehid, otherid;
	if(sscanf(params, "ud", otherid, vehid)) return Usage(playerid, "/givepv [playerid/name] [vehid] | /v my(myv) - for find vehid");
	if(vehid == INVALID_VEHICLE_ID) return Error(playerid, "Invalid id");
		...
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				if(vehid == nearid)
				{
					...
					Info(otherid, "%s Telah memberikan kendaraan %s(%d) kepada anda.(/myv)", ReturnName(playerid), GetVehicleName(vehid), GetVehicleModel(vehid));
					new str[150];
...
CMD:buysparepart(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_SPAREPART, DIALOG_STYLE_TABLIST, "Sparepart Shop", "Sparepart\t{3BBD44}Rp.500.000\n{ffffff}Vehicle Repair\t( Membutuhkan Sparepart )", "Select", "Close");
	return 1;
}
