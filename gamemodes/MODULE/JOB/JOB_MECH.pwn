...
CMD:mechduty(playerid, params[])
{
	...
	{		
		if(pData[playerid][pMechDuty] == 0)
		{
			...
			SendClientMessageToAllEx(COLOR_GREEN, "{ffff00} [MEKANIK] {ff0000} %s {ffffff}SEDANG AKTIF {ffffff}INFO KONTAK{ffff00} [NOMOR : %d]{ffffff} HUBUNGI JIKA MEMERLUKAN.", pData[playerid][pName], pData[playerid][pPhone]);
		}
		else
		{
	...
	else return Error(playerid, "Anda bukan bagian mekanik!");
	return 1;
}
