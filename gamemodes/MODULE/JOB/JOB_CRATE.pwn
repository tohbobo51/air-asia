...
CMD:getcrate(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 1.5, 2836.23, -1540.68, 11.09))
    {
		if(CrateFish != 0)
		{
			...
			SetPlayerAttachedObject(playerid, 9, 2912, 1, 0.436000, 0.519000, -0.342000, 0.000000, 0.000000, 0.000000, 1.000000,1.000000,1.000000);
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Fish with price "LG_E"$50"WHITE_E", put in your car use '/loadfish'");
    ...
    if(IsPlayerInRangeOfPoint(playerid, 1.5, 365.92, 885.47, 21.28))
    {
		if(CrateComponent != 0)
		{
		    if(CarryCrate[playerid] == 1) return SCM(playerid, COLOR_YELLOW, "WARNING: "WHITE_E"Kamu sudah membawa Crates, Tidak dapat membawa lagi");
			...
			ApplyAnimation(playerid, "CARRY", "crry_prtial", 4.1, 1, 1, 1, 1, 1, 1);
			SetPlayerAttachedObject(playerid, 9, 2912, 1, 0.436000, 0.519000, -0.342000, 0.000000, 0.000000, 0.000000, 1.000000,1.000000,1.000000);
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Component with price "LG_E"$90"WHITE_E", put in your car use '/loadcompo'");
    ...
    if(IsPlayerInRangeOfPoint(playerid, 1.5, -1425.71, -1528.95, 102.13))
    {
		if(CrateMaterial != 0)
		{
			...
			ApplyAnimation(playerid, "CARRY", "crry_prtial", 4.1, 1, 1, 1, 1, 1, 1);
			SetPlayerAttachedObject(playerid, 9, 2912, 1, 0.436000, 0.519000, -0.342000, 0.000000, 0.000000, 0.000000, 1.000000,1.000000,1.000000);
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Material with price "LG_E"$70"WHITE_E", put in your car use '/loadmats'");
...
CMD:sellcrate(playerid, params[])
{
	...
	{
		// Fish
	    if(IsPlayerInRangeOfPoint(playerid, 2.0, -371.83, -1428.88, 25.72))
		{
			...
			ClearAnimations(playerid);
			RemovePlayerAttachedObject(playerid,9);
			pData[playerid][pJobTime] = 1650;
			new String[2280];
			format(String, sizeof(String), "JOB: "WHITE_E"You have "LG_E"succesfully "WHITE_E"selling Crates(Fish). in your payment in "YELLOW_E"'/salary.");
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Fish)", 100);
			CarryCrate[playerid] =0;
			CarryCrateFish[playerid] =0;
		...
		if(IsPlayerInRangeOfPoint(playerid, 2.0, 797.60, -617.48, 16.00))
		{
			...
			pData[playerid][pJobTime] = 1650;
			new String[2280];
			format(String, sizeof(String), "JOB: "WHITE_E"You have "LG_E"succesfully "WHITE_E"selling Crates(Component). in your payment in "YELLOW_E"'/salary.");
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Component)", 250);
			CarryCrate[playerid] =0;
			CarryCrateCompo[playerid] =0;
			MaxGetCrate[playerid] =0;
			Component += 50;
		...
		// Material
		if(IsPlayerInRangeOfPoint(playerid, 2.0, -258.51, -2183.10, 29.01))
		{
			if(CarryCrateFish[playerid] == 1) return Error(playerid, "Kamu tidak dapat menjual Crates Fish di sini");
			...
			pData[playerid][pJobTime] = 1650;
			new String[2280];
			format(String, sizeof(String), "JOB: "WHITE_E"You have "LG_E"succesfully "WHITE_E"selling Crates(Material). in your payment in "YELLOW_E"'/salary.");
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Material)", 200);
			CarryCrate[playerid] =0;
			CarryCrateMats[playerid] =0;
			MaxGetCrate[playerid] =0;
			Material += 50;
