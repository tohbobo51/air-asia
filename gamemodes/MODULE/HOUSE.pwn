...
House_Type(houseid)
{
	if(hData[houseid][hType] == 1)
	{
	    switch(random(3))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = -1024.56;
				hData[houseid][hIntposY] = -1506.51;
				hData[houseid][hIntposZ] = -12.2704;
	...
	if(hData[houseid][hType] == 2)
	{
	    switch(random(5))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = -15.4287;
				hData[houseid][hIntposY] = -496.438;
				hData[houseid][hIntposZ] = -32.0943;
	...
	if(hData[houseid][hType] == 3)
	{
	    switch(random(2))
		{
			...
			case 0:
			{
				hData[houseid][hIntposX] = -1315.31;
				hData[houseid][hIntposY] = 747.658;
				hData[houseid][hIntposZ] = 1058.79;
	...
	if(hData[houseid][hType] == 4)
	{
	    switch(random(2))
		{
			case 0:
			{
				hData[houseid][hIntposX] = -603.79;
				hData[houseid][hIntposY] = 472.63;
				hData[houseid][hIntposZ] = 25.32;
...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
		...
		{
			format(string, sizeof(string), "[ID: %d]\n{FFFFFF}Lokasi Rumah {FFFF00}%s\n{FFFFFF}Type Rumah {FFFF00}%s\n"WHITE_E"Pemilik Rumah %s\nPajak: %s\nTekan '{FF0000}ENTER{FFFFFF}' Untuk Masuk", houseid, GetLocation(hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]), type, hData[houseid][hOwner], ReturnTimelapse(gettime(), hData[houseid][hVisit]));
...
CMD:buyhouse(playerid, params[])
{
	foreach(new hid : Houses)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, hData[hid][hExtpos][0], hData[hid][hExtpos][1], hData[hid][hExtpos][2]))
		{
			...
			{
			    #if LIMIT_PER_PLAYER > 0
				if(Player_HouseCount(playerid) + 1 > 5) return Error(playerid, "You can't buy any more houses.");
				#endif
			}
...
CMD:givehouse(playerid, params[])
{
	...
	{
		#if LIMIT_PER_PLAYER > 0
		if(Player_HouseCount(otherid) + 1 > 5) return Error(playerid, "Target player cant own any more houses.");
		#endif
	}
