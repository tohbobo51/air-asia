...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
					...
					if(pData[playerid][pJob] == 0 || pData[playerid][pJob2] == 0)
					{
						if(pData[playerid][pJob] == 0)
						{
							if(pData[playerid][pJob] == 0 && GetPlayerState(playerid) == 1)
							{
								...
								Info(playerid, "Anda telah berhasil mendaftarkan Job Penambang. /accept job untuk konfirmasi.");
							}
							else return Error(playerid, "Anda sudah memiliki job atau tidak berada di dekat pendaftaran job.");
						}
						else if(pData[playerid][pJob2] == 0)
	...
	if(dialogid == DIALOG_EMS)
	{
		if (response)
		{
		    ...
		    ClearAnimations(p2);
			ApplyAnimation(p2, "CARRY", "crry_prtial", 4.0, 0, 0, 0, 0, 0, 1);
			DestroyDynamic3DTextLabel(pData[p2][pInjuredLabel]);
		    TogglePlayerControllable(p2, 1);
		    SetPVarInt(playerid, "gcPlayer", p2);
		    ApplyAnimation(playerid,"MEDIC","CPR",4.1, 0, 1, 1, 1, 1, 1);
		    SetPlayerHealthEx(p2, 100.0);
		    SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s menyembuhkan segala luka %s.", ReturnName(playerid), ReturnName(p2));
		    Info(p2, "%s has revived you.", pData[playerid][pAdminname]);
		    return TogglePlayerControllable(playerid, 1);
		}
	}
 	if(dialogid == DIALOG_EMSG)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);

   			if(!NearPlayer(playerid, p2, 5.0))
        		return Error(playerid, "Kamu harus didekat Player.");

		    if(!pData[p2][pInjured])
		        return Error(playerid, "kamu tidak bisa drag orang yang tidak mati.");

		    SetPVarInt(p2, "DragBy", playerid);
		    Info(p2, "%s Telah menawari drag kepada anda, /accept drag untuk menerimanya /deny drag untuk membatalkannya.", ReturnName(playerid));
			Info(playerid, "Anda berhasil menawari drag kepada player %s", ReturnName(p2));
		    return 1;
		}
	}
 	if(dialogid == DIALOG_EMSC)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);

      		pData[p2][pInsOffer] = playerid;

		    Info(p2, "%s has offered to inspect you (type \"/accept inspect or /deny inspect\").", ReturnName(playerid));
		    Info(playerid, "You have offered to inspect %s.", ReturnName(p2));
		    return 1;
		}
	}
	if(dialogid == DIALOG_EMS1)
	{
		if (response)
		{
      		switch (listitem)
			{
				case 0:
				{
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMS, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
				case 1:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMSG, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
				case 2:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMSC, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
    			case 3:
				{
					Error(playerid, "Perkembangan!");
				}
				case 4:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMS2, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
				case 5:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMS3, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
				case 6:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMS4, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
				case 7:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_EMS5, DIALOG_STYLE_LIST, "AsiaPride - SAMD Panel", str, "Pilih", "Tutup");
				}
			}
		}
	}
	if(dialogid == DIALOG_EMS2)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);
			...
			TogglePlayerControllable(p2, 0);
		   	Info(playerid, "Kamu sedang mentreatment %s!", ReturnName(p2));
			Info(p2, "Kamu sedang ditreatment oleh medis, Mohon tenang!");
		   	//ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
			pData[playerid][pProductingStatus] = 1;
		   	pData[playerid][pProducting] = SetTimerEx("TreatmentPlayer", 1000, true, "iid", playerid, p2, 1);
	...
	if(dialogid == DIALOG_EMS3)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem), Float:health;
			...
			pData[p2][pRFoot] += 60;
			pData[p2][pLFoot] += 60;
			pData[p2][pSickTime] = 0;
		    ClearAnimations(p2);
			SetPlayerDrunkLevel(p2, 0);

			SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s has given medicine to %s with the right hand.", ReturnName(playerid), ReturnName(p2));
		    Info(p2, "Officer %s has resalve your sick character.", ReturnName(playerid));
		    return 1;
		}
	}
	if(dialogid == DIALOG_EMS4)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);
		    ...
		    if(GetVehicleModel(i) == 416)
		    {
		        ...
		        PutPlayerInVehicle(p2, i, seatid);
		        SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s opens up the ambulance and loads %s on the stretcher.", ReturnName(playerid), ReturnName(p2));

		        TogglePlayerControllable(p2, 0);
		        SetPlayerHealth(p2, 100.0);
		        Info(p2, "You're injured ~r~now you're on ambulance.");
		        return 1;
		    }
		...
		if (response)
		{
            switch (listitem)
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - TentangPonsel", gstr, "Tutup","");
				}
				case 1:
				{
	...
	if(dialogid == DIALOG_EMS5)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);

   			if(pData[p2][pInjured] != 2)
		        return Error(playerid, "That player is not injured.");

		    if(IsPlayerInRangeOfPoint(playerid, 5.0, 1142.38, -1330.74, 13.62))
		    {
				RemovePlayerFromVehicle(p2);
				pData[p2][pHospital] = 1;
				pData[p2][pHospitalTime] = 0;
				pData[p2][pInjured] = 1;
				ResetPlayerWeaponsEx(p2);
		        Info(playerid, "You have delivered %s to the hospital.", ReturnName(p2));
		        Info(p2, "You have recovered at the nearest hospital by officer %s.", ReturnName(playerid));
		    }
		...
		if (response)
		{
      		switch (listitem)
			{
				case 0:
				{
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD1, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
				case 1:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD2, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
				case 2:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD3, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
				case 3:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD4, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
				case 4:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD5, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
				case 5:
				{
					new str[500], count = 0;
					...
					if(!count) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", "Tidak ada player disekitarmu", "Tutup", ""), PlayerPlaySound(playerid, 1055, 0.0, 0.0, 0.0);
					else ShowPlayerDialog(playerid, DIALOG_SAPD6, DIALOG_STYLE_LIST, "AsiaPride - SAPD Panel", str, "Pilih", "Tutup");
				}
			}
		}
	}
	if(dialogid == DIALOG_SAPD1)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);
   			...
   			if(GetPlayerState(p2) != PLAYER_STATE_ONFOOT)
	...
	if(dialogid == DIALOG_SAPD5)
	{
		if (response)
		{
            new p2 = GetPlayerListitemValue(playerid, listitem);

   			pData[p2][pFriskOffer] = playerid;

		    Info(p2, "%s has offered to frisk you (type \"/accept frisk or /deny frisk\").", ReturnName(playerid));
		    Info(playerid, "You have offered to frisk %s.", ReturnName(p2));
		    return 1;
		}
	}
	if(dialogid == DIALOG_SAPD6)
	{
		if (response)
		{
            //new p2 = GetPlayerListitemValue(playerid, listitem);
   			Info(playerid, "Fitur ini dalam tahap pengembangan...");
		    return 1;
		}
	}
 	if(dialogid == DIALOG_AMOUNT)
	{
		if (response)
		{
			...
			PlayerTextDrawSetString(playerid, JUMLAH[playerid], str);
		}
	}
 	if(dialogid == DIALOG_GIVE)
	{
		if (response)
		{
			...
			/*if (sscanf(inputtext, "u", userid))
				return ShowPlayerDialog(playerid, DIALOG_GIVE, DIALOG_STYLE_INPUT, "Memberikan Barang", "Kantong Berapa Yang Ingin Di Berikan:", "Berikan", "Batal"), PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);

			if (userid == INVALID_PLAYER_ID)
				return ShowPlayerDialog(playerid, DIALOG_GIVE, DIALOG_STYLE_INPUT, "Memberikan Barang", "KESALAHAN: Kantong Player Tidak Ada.\n\nKantong Berapa Yang Ingin Di Berikan:", "Berikan", "Batal"), PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);

			if(!IsPlayerConnected(userid) || !NearPlayer(playerid, userid, 5.0))
				return ShowPlayerDialog(playerid, DIALOG_GIVE, DIALOG_STYLE_INPUT, "Memberikan Barang", "KESALAHAN: Player Tersebut Tidak Berada Di Dekat Kamu.\n\nKantong Berapa Yang Ingin Di Berikan:", "Berikan", "Batal"), PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);

			if (userid == playerid)
				return ShowPlayerDialog(playerid, DIALOG_GIVE, DIALOG_STYLE_INPUT, "Memberikan Barang", "KESALAHAN: Kamu Tidak Bisa Memberikan Ke Diri Sendiri.\n\nKantong Berapa Yang Ingin Di Berikan:", "Berikan", "Batal"), PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);*/

			itemid = pData[playerid][pSelectItem];
			...
			if(Inventory_Count(playerid, string) < pData[playerid][pGiveAmount])
				return Error(playerid, "KESALAHAN: Barang Kamu Tidak Mencukupi !");

			for (new i = 0; i < sizeof(g_aInventoryItems); i ++) if (!strcmp(g_aInventoryItems[i][e_InventoryItem], string, true))
			{
				new id = Inventory_Addset(p2, string, InventoryData[playerid][itemid][invModel], pData[playerid][pGiveAmount]);

				if (id == -1)
				return Error(playerid, "KESALAHAN: Player Tersebut Tidak Memiliki Slot Di Inventorynya !");
				
				if(InventoryData[playerid][itemid][invModel] == 2821)
				{
					...
					ShowItemBox(p2, "Snack", str, 2821, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			
						
					pData[playerid][pSnack] -= pData[playerid][pGiveAmount];
					pData[p2][pSnack] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 2958)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Air", str, 2958, 2);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Air", str, 2958, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			
					
					pData[playerid][pSprunk] -= pData[playerid][pGiveAmount];
					pData[p2][pSprunk] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 1578)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Marijuana", str, 1578, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Marijuana", str, 1578, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			
					
					pData[playerid][pMarijuana] -= pData[playerid][pGiveAmount];
					pData[p2][pMarijuana] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 1212)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Red Money", str, 1212, 2);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Red Money", str, 1212, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pRedMoney] -= pData[playerid][pGiveAmount];
					pData[p2][pRedMoney] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19473)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Sparepart", str, 19473, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Sparepart", str, 19473, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pSparepart] -= pData[playerid][pGiveAmount];
					pData[p2][pSparepart] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 18641)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Flashlight", str, 18641, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Flashlight", str, 18641, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pFlashlight] -= pData[playerid][pGiveAmount];
					pData[p2][pFlashlight] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 11746)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Lockpick", str, 11746, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Lockpick", str, 11746, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pLockPick] -= pData[playerid][pGiveAmount];
					pData[p2][pLockPick] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 11738)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Perban", str, 11738, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Perban", str, 11738, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pBandage] -= pData[playerid][pGiveAmount];
					pData[p2][pBandage] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19182)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Medicine", str, 19182, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Medicine", str, 19182, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pMedicine] -= pData[playerid][pGiveAmount];
					pData[p2][pMedicine] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 11736)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Medkit", str, 11736, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Medkit", str, 11736, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pMedkit] -= pData[playerid][pGiveAmount];
					pData[p2][pMedkit] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19621)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Bensin", str, 19621, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Bensin", str, 19621, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pGas] -= pData[playerid][pGiveAmount];
					pData[p2][pGas] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2041)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Material", str, 2041, 2);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Material", str, 2041, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pMaterial] -= pData[playerid][pGiveAmount];
					pData[p2][pMaterial] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 3096)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Component", str, 3096, 4);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Component", str, 3096, 4);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pComponent] -= pData[playerid][pGiveAmount];
					pData[p2][pComponent] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19567)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Food", str, 19567, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Food", str, 19567, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pFood] -= pData[playerid][pGiveAmount];
					pData[p2][pFood] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 1241)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Obat", str, 1241, 4);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Obat", str, 1241, 4);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pObat] -= pData[playerid][pGiveAmount];
					pData[p2][pObat] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 862)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Padi", str, 862, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Padi", str, 862, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pSeed] -= pData[playerid][pGiveAmount];
					pData[p2][pSeed] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 18845)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Kentang", str, 18845, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Kentang", str, 18845, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pPotato] -= pData[playerid][pGiveAmount];
					pData[p2][pPotato] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 818)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "wheat", str, 818, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "wheat", str, 818, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pWheat] -= pData[playerid][pGiveAmount];
					pData[p2][pWheat] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19574)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Jeruk", str, 19574, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Jeruk", str, 19574, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pOrange] -= pData[playerid][pGiveAmount];
					pData[p2][pOrange] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19630)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Ikan", str, 19630, 2);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Ikan", str, 19630, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pFish] -= pData[playerid][pGiveAmount];
					pData[p2][pFish] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19078)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Ayam Hidup", str, 19078, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Ayam Hidup", str, 19078, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][AyamHidup] -= pData[playerid][pGiveAmount];
					pData[p2][AyamHidup] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2806)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Daging Ayam", str, 2806, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Daging Ayam", str, 2806, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][AyamPotong] -= pData[playerid][pGiveAmount];
					pData[p2][AyamPotong] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2768)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Ayam Bungkus", str, 2768, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Ayam Bungkus", str, 2768, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][AyamFillet] -= pData[playerid][pGiveAmount];
					pData[p2][AyamFillet] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2663)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Nasi", str, 2663, 2);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Nasi", str, 2663, 2);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pNasi] -= pData[playerid][pGiveAmount];
					pData[p2][pNasi] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19882)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Ikan Goreng", str, 19882, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Ikan Goreng", str, 19882, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pAGoreng] -= pData[playerid][pGiveAmount];
					pData[p2][pAGoreng] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2703)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Burger", str, 2703, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Burger", str, 2703, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pBurger] -= pData[playerid][pGiveAmount];
					pData[p2][pBurger] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 2769)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Kebab", str, 2769, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Kebab", str, 2769, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pKebab] -= pData[playerid][pGiveAmount];
					pData[p2][pKebab] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 19570)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Susu", str, 19570, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Susu", str, 19570, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pSusu] -= pData[playerid][pGiveAmount];
					pData[p2][pSusu] += pData[playerid][pGiveAmount];
				}
    			else if(InventoryData[playerid][itemid][invModel] == 860)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Crack", str, 860, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Crack", str, 860, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][pCrack] -= pData[playerid][pGiveAmount];
					pData[p2][pCrack] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 1303)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Batu", str, 1303, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Batu", str, 1303, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][Batu] -= pData[playerid][pGiveAmount];
					pData[p2][Batu] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 828)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Batu Cuci", str, 828, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Batu Cuci", str, 828, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][BatuCucian] -= pData[playerid][pGiveAmount];
					pData[p2][BatuCucian] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 19941)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Emas", str, 19941, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Emas", str, 19941, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][bEmas] -= pData[playerid][pGiveAmount];
					pData[p2][bEmas] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 8040)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Tembaga", str, 8040, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Tembaga", str, 8040, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][bTembaga] -= pData[playerid][pGiveAmount];
					pData[p2][bTembaga] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 8841)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Alumunium", str, 8841, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Alumunium", str, 8841, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			

					pData[playerid][bAlumunium] -= pData[playerid][pGiveAmount];
					pData[p2][bAlumunium] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 19897)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Rokok", str, 19897, 3);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Rokok", str, 19897, 3);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			
					
					pData[playerid][pRokok] -= pData[playerid][pGiveAmount];
					pData[p2][pRokok] += pData[playerid][pGiveAmount];
				}
				else if(InventoryData[playerid][itemid][invModel] == 1581)
				{
				    format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(playerid, "Tikettol", str, 1581, 1);
					
					format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
					ShowItemBox(p2, "Tikettol", str, 1581, 1);

					ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
					ApplyAnimation(p2, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);			
					
					pData[playerid][pTikettol] -= pData[playerid][pGiveAmount];
					pData[p2][pTikettol] += pData[playerid][pGiveAmount];
				}
			}
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			else 
			{
				...
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login to AsiaPride Roleplay", lstring, "Login", "Quit");
			}
		}
        return 1;
    }
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration AsiaPride Roleplay", "Kata sandi minimal 5 Karakter!\nMohon isi Password dibawah ini:", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration AsiaPride Roleplay", "Kata sandi yang anda gunakan mengandung karakter yang valid!\nMohon isi Password dibawah ini:", "Register", "Tolak");
			return 1;
		}
	...
	if(dialogid == DIALOG_MAKE_CHAR)
	{
	    if(response)
	    {
		    if(strlen(inputtext) < 1 || strlen(inputtext) > 24)
				return ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, "Create Character", "Insert your new Character Name\n\nExample: Clairo_Stevano, Andrea_Martinez etc.", "Create", "Back");
    ...
    {
		if(!response) return ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Masukan tanggal lahir (Tgl/Bulan/Tahun): 15/04/1990", "Pilih", "Batal");
		if(response)
		{
			new
				iDay,
		...
		if(response)
		{
			switch(listitem)
			{
				...
				//		TextDrawHideForPlayer(playerid, StatsTD[txd]);
						PlayerTextDrawHide(playerid, StatsTD[playerid][txd]);
					}
...
//					ShowPlayerProgressBarss(playerid, pData[playerid][sphungrybar]);
			...
			switch(listitem)
			{
				...
				case 1:
				{
	                if(pData[playerid][IsLoggedIn] == true)
					{
						...
						PlayerTextDrawColor(playerid, VoiceTD[playerid][1], COLOR_LBLUE);
						PlayerTextDrawShow(playerid, VoiceTD[playerid][1]);
		             	if ((lstream[playerid] = SvCreateDLStreamAtPlayer(20.0, SV_INFINITY, playerid, 0xff0000ff, "Normal")))
				...
				case 2:
				{
	  	            if(pData[playerid][IsLoggedIn] == true)
					{
						...
						PlayerTextDrawShow(playerid, VoiceTD[playerid][1]);
		             	if ((lstream[playerid] = SvCreateDLStreamAtPlayer(60.0, SV_INFINITY, playerid, 0xff0000ff, "Teriak")))
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					PlayerTextDrawShow(playerid, VoiceTD[playerid][1]);
                    if ((lstream[playerid] = SvCreateDLStreamAtPlayer(5.0, SV_INFINITY, playerid, 0xff0000ff, "Berbisik")))
	             	SuccesMsg(playerid, "Setting suaramu berbisik.");
				}
				case 1:
				{
					PlayerTextDrawHide(playerid, VoiceTD[playerid][1]);
					PlayerTextDrawSetString(playerid, VoiceTD[playerid][1], "NORMAL");
					PlayerTextDrawColor(playerid, VoiceTD[playerid][1], COLOR_LENZ);
					PlayerTextDrawShow(playerid, VoiceTD[playerid][1]);
	             	if ((lstream[playerid] = SvCreateDLStreamAtPlayer(15.0, SV_INFINITY, playerid, 0xff0000ff, "Normal")))
	                SuccesMsg(playerid, "Setting suaramu normal.");
				}
				case 2:
				{
					PlayerTextDrawHide(playerid, VoiceTD[playerid][1]);
		            ...
		            if ((lstream[playerid] = SvCreateDLStreamAtPlayer(40.0, SV_INFINITY, playerid, 0xff0000ff, "Teriak")))
	  	            SuccesMsg(playerid, "Setting suaramu teriak.");
				}
			}
		}
		return 1;
	...
	if(dialogid == ST_MENU)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialog(playerid, ST_COMPONENT, DIALOG_STYLE_LIST, "Gudang Component", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, ST_MATERIAL, DIALOG_STYLE_LIST, "Gudang Material", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 2:
				{
					ShowPlayerDialog(playerid, ST_IKAN, DIALOG_STYLE_LIST, "Gudang Ikan", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 3:
				{
					ShowPlayerDialog(playerid, ST_REDMONEY, DIALOG_STYLE_LIST, "Gudang Uang Merah", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 4:
				{
					ShowPlayerDialog(playerid, ST_BORAX, DIALOG_STYLE_LIST, "Gudang Marijuana", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 5:
				{
					ShowPlayerDialog(playerid, ST_MONEY, DIALOG_STYLE_LIST, "Gudang Money", "Withdraw\nDeposit", "Select","Cancel");
				}
			}
		}
	}
	if(dialogid == ST_BORAX)
	{
		if(response)
		{
			new str[256], stid = pData[playerid][pInSt];
			switch(listitem)
			{
				case 0:
				{
					pData[playerid][pMenuType] = 1;
					format(str, sizeof str,"Marijuana: %d\n\nSilakan Masukkan jumlah untuk Mengambil", stData[stid][stMarijuana]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Marijuana: %d\n\nSilakan Masukan Jumlah untuk Meletakkan nya", stData[stid][stMarijuana]);
				}
			}
			ShowPlayerDialog(playerid, ST_BORAX2, DIALOG_STYLE_INPUT, "Marijuana Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_BORAX2)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			if(pData[playerid][pMenuType] == 1)
			{
				if(amount < 1)
					return ErrorMsg(playerid, "Minimum amount is 1");

				if(stData[stid][stMarijuana] < amount) return ErrorMsg(playerid, "Tidak Cukup Borax");

				if((pData[playerid][pMarijuana] + amount) >= 501)
					return ErrorMsg(playerid, "Anda telah mencapai maksimum Marijuana");

				pData[playerid][pMarijuana] += amount;
				stData[stid][stMarijuana] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil Marijuana sebanyak %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				...
				if(pData[playerid][pMarijuana] < amount) return ErrorMsg(playerid, "Tidak Cukup Marijuana");

				if((stData[stid][stMarijuana] + amount) >= MAX_STORAGE_INT)
					return ErrorMsg(playerid, "Anda telah mencapai maksimum Marijuana");

				pData[playerid][pMarijuana] -= amount;
				stData[stid][stMarijuana] += amount;
				Storage_Save(stid);
				Info(playerid, "Anda menaruh Component sebanyak %d", amount);
			}
		}
	}
	if(dialogid == ST_REDMONEY)
	{
		if(response)
		{
			new str[256], stid = pData[playerid][pInSt];
			switch(listitem)
			{
				...
				{
					pData[playerid][pMenuType] = 1;
					format(str, sizeof str,"Uang Merah: %d\n\nSilakan Masukkan jumlah untuk Mengambil", stData[stid][stUangMerah]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Uang Merah: %d\n\nSilakan Masukan Jumlah untuk Meletakkan nya", stData[stid][stUangMerah]);
				}
			}
			ShowPlayerDialog(playerid, ST_REDMONEY2, DIALOG_STYLE_INPUT, "Uang Merah Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_REDMONEY2)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stUangMerah] < amount) return ErrorMsg(playerid, "Tidak Cukup Uang Merah");

				if((pData[playerid][pRedMoney] + amount) >= 501)
					return ErrorMsg(playerid, "Anda telah mencapai maksimum Uang Merah");

				pData[playerid][pRedMoney] += amount;
				stData[stid][stUangMerah] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil Uang Merah sebanyak %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				if(amount < 1)
				...
				if(pData[playerid][pRedMoney] < amount) return ErrorMsg(playerid, "Tidak Cukup Uang Merah");

				if((stData[stid][stUangMerah] + amount) >= MAX_STORAGE_REDMONEY)
					return ErrorMsg(playerid, "Anda telah mencapai maksimum Uang Merah");
	...
	if(dialogid == ST_IKAN)
	{
		if(response)
		{
			...
			switch(listitem)
			{
				...
				{
					pData[playerid][pMenuType] = 1;
					format(str, sizeof str,"Ikan: %d\n\nPlease Input amount to Withdraw", stData[stid][stFish]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Ikan: %d\n\nPlease Input amount to Deposit", stData[stid][stFish]);
				}
			}
			ShowPlayerDialog(playerid, ST_IKAN2, DIALOG_STYLE_INPUT, "Ikan Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_IKAN2)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stFish] < amount) return ErrorMsg(playerid, "Not Enough Fish.");

				if(Inventory_Count(playerid, "Ikan") > 500) return ErrorMsg(playerid, "You've reached maximum of Fish");

				pData[playerid][pFish] += amount;
				stData[stid][stFish] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil Ikan Sebesar %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				if(amount < 1)
				...
				if(Inventory_Count(playerid, "Ikan") < 1) return ErrorMsg(playerid, "Kamu tidak memiliki Ikan.");

				if((stData[stid][stFish] + amount) >= MAX_STORAGE_INT)
					return ErrorMsg(playerid, "You've reached maximum of Fish");

				pData[playerid][pFish] -= amount;
				stData[stid][stFish] += amount;
				Storage_Save(stid);
				Info(playerid, "Anda menaruh Ikan sebesar %d", amount);
			}
		}
	}
	if(dialogid == ST_COMPONENT)
	{
		if(response)
		{
			new str[256], stid = pData[playerid][pInSt];
			switch(listitem)
			{
				...
				{
					pData[playerid][pMenuType] = 1;
					format(str, sizeof str,"Component: %d\n\nPlease Input amount to Withdraw", stData[stid][stComponent]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Component: %d\n\nPlease Input amount to Deposit", stData[stid][stComponent]);
				}
			}
			ShowPlayerDialog(playerid, ST_COMPONENT2, DIALOG_STYLE_INPUT, "Component Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_COMPONENT2)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stComponent] < amount) return ErrorMsg(playerid, "Not Enough Workshop Component");

				if(Inventory_Count(playerid, "Component") > 500) return ErrorMsg(playerid, "You've reached maximum of Component");

				pData[playerid][pComponent] += amount;
				stData[stid][stComponent] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil Component Sebesar %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				if(amount < 1)
				...
				if(Inventory_Count(playerid, "Component") < 1) return ErrorMsg(playerid, "Kamu tidak memiliki Component.");

				if((stData[stid][stComponent] + amount) >= MAX_STORAGE_INT)
					return ErrorMsg(playerid, "You've reached maximum of Component");

				pData[playerid][pComponent] -= amount;
				stData[stid][stComponent] += amount;
				Storage_Save(stid);
				Info(playerid, "Anda menaruh Component sebesar %d", amount);
			}
		}
	}
	if(dialogid == ST_MATERIAL)
	{
		if(response)
		{
			new str[256], stid = pData[playerid][pInSt];
			switch(listitem)
			{
				...
				{
					pData[playerid][pMenuType] = 1;
					format(str, sizeof str,"Material: %d\n\nPlease Input amount to Withdraw", stData[stid][stMaterial]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Material: %d\n\nPlease Input amount to Deposit", stData[stid][stMaterial]);
				}
			}
			ShowPlayerDialog(playerid, ST_MATERIAL2, DIALOG_STYLE_INPUT, "Material Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_MATERIAL2)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stMaterial] < amount) return ErrorMsg(playerid, "Not Enough Material");

				pData[playerid][pMaterial] += amount;
				stData[stid][stMaterial] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil material sebesar %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				if(amount < 1)
				...
				if(Inventory_Count(playerid, "Material") < amount) return ErrorMsg(playerid, "Material anda kurang!");

				if((stData[stid][stMaterial] + amount) >= MAX_STORAGE_INT)
					return ErrorMsg(playerid, "You've reached maximum of Material");

				pData[playerid][pMaterial] -= amount;
				stData[stid][stMaterial] += amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil menaruh material sebesar %d", amount);
			}
		}
	}
	if(dialogid == ST_MONEY)
	{
		if(response)
		{
			new str[264], stid = pData[playerid][pInSt];
			switch(listitem)
			{
				case 0:
				{
					...
					format(str, sizeof str, "Money:\n{7fff00}%s\n\n{ffffff}Input Amount to Withdraw", FormatMoney(stData[stid][stMoney]));
					ShowPlayerDialog(playerid, ST_WITHDRAWMONEY, DIALOG_STYLE_INPUT, "Withdraw Money",str,"Withdraw","Cancel");
				}
				case 1:
				{
					format(str, sizeof str, "Money:\n{7fff00}%s\n\n{ffffff}Input Amount to Deposit", FormatMoney(stData[stid][stMoney]));
					ShowPlayerDialog(playerid, ST_DEPOSITMONEY, DIALOG_STYLE_INPUT, "Deposit Money",str,"Deposit","Cancel");
				}
			}
		}
	}
	if(dialogid == ST_WITHDRAWMONEY)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			...
			if(stData[stid][stMoney] < amount)
				return ErrorMsg(playerid, "Not Enough Money");

			GivePlayerMoneyEx(playerid, amount);
			stData[stid][stMoney] -= amount;
			Storage_Save(stid);
		}
	}
	if(dialogid == ST_DEPOSITMONEY)
	{
		if(response)
		{
			new amount = strval(inputtext), stid = pData[playerid][pInSt];
			...
			switch(listitem)
			{
				...
				case 4:
				{
					...
					if(bData[bid][bMoney] < 1000)
						return Error(playerid, "Setidaknya anda mempunyai uang dalam bisnis anda senilai Rp.5.000.000 untuk merestock product.");
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"AsiaPride Roleplay: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						if(price == 0)
			...
			else if(bData[bizid][bType] == 4)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						else return Error(playerid, "Kamu tidak memiliki licensi senjata!");
					}
					case 2:
					{
						if(price == 0)
					...
					case 4:
					{
						...
						else return Error(playerid, "Kami tidak memiliki licensi penebang!");
					}
					case 5:
					{
						if(price == 0)
					...
					case 6:
					{
						...
						if(pData[playerid][pFishTool] > 1) return Error(playerid, "Kamu hanya bisa membeli 1 pancingan!");
						GivePlayerMoneyEx(playerid, -price);
	...
	if(dialogid == BISNIS_PRICESET)
	{
		...
		if(Player_OwnsBisnis(playerid, pData[playerid][pInBiz]))
		{
			if(response)
			{
				...
				{
					new str[128];
					format(str,sizeof(str), "Please enter the new product price for %s (Rp.1 to Rp.500,000):", item);
					ShowPlayerDialog(playerid, BISNIS_PRICESET, DIALOG_STYLE_INPUT, "Business: Set Price", str, "Modify", "Back");
					return 1;
				}
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""RED_E"AsiaPride Roleplay: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 3:
				{
...
//-----------[ Gov Commands Dialog ]----------
		...
		if(response)
  		{
			...
			if(sscanf(inputtext, "s[128]", text))
      		...
      		{
      		    new AgusSamp[128];
                TextDrawShowForPlayer(i, Polisi[0]);
                TextDrawShowForPlayer(i, Polisi[1]);
                TextDrawShowForPlayer(i, Polisi[2]);
                TextDrawShowForPlayer(i, Polisi[3]);
                TextDrawShowForPlayer(i, Polisi[4]);
                TextDrawShowForPlayer(i, Polisi[5]);
                TextDrawShowForPlayer(i, Polisi[6]);
                TextDrawShowForPlayer(i, Polisi[7]);
                TextDrawShowForPlayer(i, Polisi[8]);
                TextDrawShowForPlayer(i, Polisi[9]);
                TextDrawShowForPlayer(i, Polisi[10]);
                TextDrawShowForPlayer(i, Polisi[11]);
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, TextPolisi);
	...
	if(dialogid == DIALOG_HP_ABANGKU)
    {
        if(response)
        {
            ...
            if(sscanf(inputtext, "s[128]", text))
            {
                return ShowPlayerDialog(playerid, DIALOG_HP_FACEBOOK, DIALOG_STYLE_INPUT, "Posting {33AAFF}FACEBOOK", "Masukan Text Apa Yang Mau Di Posting Di {33AAFF}FACEBOOK", "Posting", "Batal");
            }
            for(new i; i <MAX_PLAYERS; i++)
            {
                new AgusSamp[128];
                ...
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, Abangku);
            	PlayerTextDrawShow(i, TWTNOTIFTD[i][0]);
                PlayerTextDrawShow(i, TWTNOTIFTD[i][1]);
                ...
                RmTimer[i] = SetTimerEx("RmOffline", 9000, true, "d", i);
            }
        }
    }
    if(dialogid == DIALOG_SKIBIDI)
    {
        if(response)
        {
            new text[128];
            ...
            for(new i; i <MAX_PLAYERS; i++)
            {
                ...
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, Skibidi);
            	PlayerTextDrawShow(i, TWTNOTIFTD[i][0]);
                PlayerTextDrawShow(i, TWTNOTIFTD[i][1]);
                ...
                AuTimer[i] = SetTimerEx("AuOffline", 9000, true, "d", i);
            }
        }
    }
    if(dialogid == DIALOG_AURORA)
    {
        if(response)
        {
            new text[128];
            ...
            for(new i; i <MAX_PLAYERS; i++)
            {
                ...
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, Aurora);
            	PlayerTextDrawShow(i, TWTNOTIFTD[i][0]);
                PlayerTextDrawShow(i, TWTNOTIFTD[i][1]);
                ...
                RkTimer[i] = SetTimerEx("RkOffline", 9000, true, "d", i);
            }
        }
    }
    if(dialogid == DIALOG_ROMAKEPALA)
    {
        if(response)
        {
            new text[128];
            ...
            for(new i; i <MAX_PLAYERS; i++)
            {
                ...
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, Romakepala);
            	PlayerTextDrawShow(i, TWTNOTIFTD[i][0]);
                PlayerTextDrawShow(i, TWTNOTIFTD[i][1]);
                ...
                SkTimer[i] = SetTimerEx("SkOffline", 9000, true, "d", i);
            }
        }
    }
    if(dialogid == DIALOG_RORMANG)
    {
        if(response)
        {
            new text[128];
            ...
            for(new i; i <MAX_PLAYERS; i++)
            {
                ...
                TextDrawShowForPlayer(i, Polisi[12]);
                TextDrawShowForPlayer(i, Polisi[13]);
                TextDrawShowForPlayer(i, Rormang);
            	PlayerTextDrawShow(i, TWTNOTIFTD[i][0]);
                PlayerTextDrawShow(i, TWTNOTIFTD[i][1]);
                ...
                FbTimer[i] = SetTimerEx("FbOffline", 9000, true, "d", i);
            }
        }
    }
	//-----------[ Player Commands Dialog ]----------
 	if(dialogid == DIALOG_STREAMER)
    {
		if (response)
		{
	        pData[playerid][pStreamer] = listitem;
	        SetPlayerStreamerSettings(playerid);
         	SendClientMessage(playerid, -1, "{C6E2FF}STREAMER: {FFFFFF}Streamer settings saved!");
	    }
		return 1;
	}
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			...
			case 2:
			{
				...
				strcat(str, ""LG_E"VEHICLE: /myv - Check Vehicles || /claimpv - Claim Insurance\n");
	...
	if(dialogid == DIALOG_JOB)
    {
		...
		switch(listitem)
		{
			...
			{
				new str[3500];
				strcat(str, "{ffffff}Pekerjaan ini dapat anda dapatkan di Flint Country\n\n{7fffd4}CMDS: /mission /cratemission /storeproduct /storegas /storestock /gps\n");
			...
			case 9:
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"Pemotong Job", str, "Close", "");
			}
			case 10:
			{
				new str[3500];
	...
	if(dialogid == DIALOG_GPS)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 3:
				{
					ShowPlayerDialog(playerid, DIALOG_GPS_JOB, DIALOG_STYLE_LIST, "GPS Jobs & Sidejobs", "Lumber Jack\nPemeras Susu\nMiner\nProduction\nFarmer\nBaggage Airport\nPemotong Ayam\nPenambang\nSweeper( SideJob )\nBus( SideJob )\nForklift( SideJob )\nMower( SideJob )", "Select", "Back");
				}
				case 4:
				{
	...
	if(dialogid == DIALOG_GPS_JOB)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					SetPlayerRaceCheckpoint(playerid,1, -514.28, -194.16, 77.90, 0.0, 0.0, 0.0, 3.5); //Lumber
					Gps(playerid, "Active!");			
				}
				case 1:
				{
					SetPlayerRaceCheckpoint(playerid,1,1050.24, -315.99, 73.39, 0.0, 0.0, 0.0, 3.5); //Susu
					Gps(playerid, "Active!");
				}
				case 2:
				{
				...
				case 6:
				{
					SetPlayerRaceCheckpoint(playerid,1, 921.77, -1287.54, 14.40, 0.0, 0.0, 0.0, 3.5); //Pemotong
					Gps(playerid, "Active!");

				}
				case 7:
				{
					ShowPlayerDialog(playerid, DIALOG_PENAMBANG, DIALOG_STYLE_LIST, "GPS Penambang", "Tambang Sulawesi", "Select", "Close");
				}
				case 8:
				{
				...
				case 9:
				{
					SetPlayerRaceCheckpoint(playerid,1, -124.33, -1511.21, 4.13, 0.0, 0.0, 0.0, 3.5); //Bus
					Gps(playerid, "Active!");
	...
	if(dialogid == DIALOG_PENAMBANG1)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					callcmd::uvfbrtyhpukik(playerid, "");
				}
				case 1:
				{
					callcmd::ebydtcyvrvnfbv(playerid, "");
				}
				case 2:
				{
					callcmd::pofcdbxvcfw(playerid, "");
				}
			}
		}
	}
	if(dialogid == DIALOG_PENAMBANG)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					SetPlayerRaceCheckpoint(playerid,1, -2071.3652, 225.5816, 35.7516, 0.0,0.0,0.0, 3.5); //Taxi
					Gps(playerid, "Active!");
				}
			}
		}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					SetPlayerRaceCheckpoint(playerid, 1, -80.886795, -1125.884765, 0.778125, 0.0, 0.0, 0.0, 3);//bank
					Gps(playerid, "Active!");
				}
				case 1:
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1101.997436, -1742.742431, 13.320490, 0.0, 0.0, 0.0, 3);//city hall
					Gps(playerid, "Active!");
				}
				case 2:
				{
	...
	if(dialogid == DIALOG_GPS_PUBLIC)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 686.482116, -1512.735595, 14.888568, 0.0, 0.0, 0.0, 3);//Dealership
					Gps(playerid, "Active!");
				}
				case 4:
				{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, -611.185302, -524.716674, 25.223440, 0.0, 0.0, 0.0, 3);//DMV
					Gps(playerid, "Active!");
				}
				case 6:
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1324.78, -885.75, 39.57, 0.0, 0.0, 0.0, 3);//Insurance
					Gps(playerid, "Active!");
				}
				case 7:
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1867.0510, -1815.1311, 14.2783, 0.0, 0.0, 0.0, 3);//Mechanic
					Gps(playerid, "Active!");
				}
	...
	if(dialogid == DIALOG_PAY)
	{
		if(response)
		{
			...
			if(otherid == playerid)
				return ErrorMsg(playerid, "kamu tidak bisa tf ke akunmu sendiri!");
		...
		if(response)
		{
			switch (listitem)
			{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_WEAPONTNI, DIALOG_STYLE_LIST, "TNI Weapons", "SPRAYCAN\nPARACHUTE\nNITE STICK\nKNIFE\nCOLT45\nSILENCED\nDEAGLE\nSHOTGUN\nSHOTGSPA\nMP5\nM4\nRIFLE\nSNIPER", "Pilih", "Batal");
				}
				case 4:
				{
					if(pData[playerid][pOnDuty] <= 0)
					...
					switch (pData[playerid][pGender])
					{
						case 1: ShowPlayerSelectionMenu(playerid, TNI_SKIN_MALE, "Choose Your Skin", TNISkinMale, sizeof(TNISkinMale));
						case 2: ShowPlayerSelectionMenu(playerid, TNI_SKIN_FEMALE, "Choose Your Skin", TNISkinFemale, sizeof(TNISkinFemale));
					}
				}
				case 5:
				{
		...
		if(response)
		{
			switch (listitem)
			{
				...
				case 1:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_GUDANGPEDAGANG, DIALOG_STYLE_LIST, "PEDAGANG", "Susu\nKebab\nNasi Bungkus\nBurger\nSprunk", "Pilih", "Batal");
				}
			}
		}
		return 1;
	...
	if(dialogid == DIALOG_LOCKERTAMBANG)
	{
		if(response)
		{
			switch(listitem)
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 5: //lockpick
				{
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat lockpick dengan 300 material!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("LockPick", 1000, true, "idd", playerid, WEAPON_SILENCED, 1);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
	...
	if(dialogid == DIALOG_MENUMASAK)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][AyamFillet] < 2) return Error(playerid, "Ayam Fillet tidak cukup!(Butuh: 2).");
					if(pData[playerid][pSnack] < 2) return Error(playerid, "Snack tidak cukup!(Butuh: 2).");

					pData[playerid][AyamFillet] -= 2;
					pData[playerid][pSnack] -= 2;

					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda memulai memasak !");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pKebab] += 5;
					pData[playerid][pArmsDealer] = SetTimerEx("Menumasak", 1000, true, "idd", playerid);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pPotato] < 2) return Error(playerid, "Kamu Membutuhkan 2 Potato.");
					if(pData[playerid][AyamFillet] < 3) return Error(playerid, "Kamu MembutuhKan 3 Ayam Fillet.");


					pData[playerid][pPotato] -= 2;
					pData[playerid][AyamFillet] -= 3;

					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda Memulai Memasak!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pNasi] += 5;
					pData[playerid][pArmsDealer] = SetTimerEx("Menumasak", 1000, true, "idd", playerid);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				...
				case 2: //colt45 9mm
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pPotato] < 2) return Error(playerid, "Kamu Membutuhkan 2 potato.");
					if(pData[playerid][pSprunk] < 2) return Error(playerid, "Kamu MembutuhKan 2 Sprunk.");


					pData[playerid][pPotato] -= 2;
					pData[playerid][pSprunk] -= 2;

					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda Memulai mengolah susu!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pSusu] += 5;
					pData[playerid][pArmsDealer] = SetTimerEx("Menumasak", 1000, true, "idd", playerid);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pPotato] < 3) return Error(playerid, "Kamu Membutuhkan 3 Potato.");
					if(pData[playerid][AyamFillet] < 3) return Error(playerid, "Kamu MembutuhKan 3 Ayamfillet.");


					pData[playerid][pPotato] -= 3;
					pData[playerid][AyamFillet] -= 3;
				...
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pFish] < 20) return Error(playerid, "Kamu Membutuhkan minimal 20 Lbs ikan.");
				//	if(pData[playerid][AyamFillet] < 3) return Error(playerid, "Kamu MembutuhKan 3 Ayamfillet.");


					pData[playerid][pFish] -= 20;
			//		pData[playerid][AyamFillet] -= 3;
	...
	if(dialogid == DIALOG_HAULING)
	{
		if(response)
		{
			...
			pData[playerid][pTrailer] = CreateVehicle(584, 326.57, 857.31, 28.40, 290.67, -1, -1, -1, 0);
	...
	if(dialogid == DIALOG_RESTOCK)
	{
		if(response)
		{
			...
			SetPlayerRaceCheckpoint(playerid,1, 365.92, 885.47, 21.28, 0.0, 0.0, 0.0, 3.5);
	...
	if(dialogid == DIALOG_GASOIL)
	{
		...
		{
			new amount = floatround(strval(inputtext));
			new value = amount * 5000;
			new vehicleid = GetPlayerVehicleID(playerid), carid = -1;
			new total = VehGasOil[vehicleid] + amount;
			
			if(amount < 300 || amount > 300) return Error(playerid, "amount maximal 300.");
			if(GetPlayerMoney(playerid) < value) return Error(playerid, "Uang anda kurang.");
			if(GasOil < amount) return Error(playerid, "GasOil stock tidak mencukupi.");
			if(total > 300) return Error(playerid, "Gas Oil Maximal 300 liter in your vehicle tank!");
			GivePlayerMoneyEx(playerid, -value);
	...
	if(dialogid == DIALOG_OBAT)
	{
		if(response)
		{
			...
			if(amount < 0 || amount > 50) return Error(playerid, "amount maximal 0 - 50.");
			if(total > 100) return Error(playerid, "Obat terlalu penuh di Inventory! Maximal 100.");
			if(GetPlayerMoney(playerid) < value) return Error(playerid, "Uang anda kurang.");
	...
	if(dialogid == DIALOG_CRACK)
	{
		if(response)
		{
			new amount = floatround(strval(inputtext));
			new total = pData[playerid][pCrack] + amount;
			new value = amount * 10000;
			if(amount < 0 || amount > 8) return Error(playerid, "amount maximal 0 - 8.");
			if(total > 8) return Error(playerid, "Crack full in your inventory! max: 8g.");
			if(GetPlayerMoney(playerid) < value) return Error(playerid, "Uang anda kurang.");
			if(Crack < amount) return Error(playerid, "Crack stock tidak mencukupi.");
			GivePlayerMoneyEx(playerid, -value);
			pData[playerid][pCrack] += amount;
			Crack -= amount;
			Server_AddMoney(value);
			Info(playerid, "Anda berhasil membeli "GREEN_E"%d "WHITE_E"Crack seharga "RED_E"%s.", amount, FormatMoney(value));
		}
	}
	...
	if(dialogid == DIALOG_ATM)
	{
		...
		switch(listitem)
		{
			...
			case 1: // Withdraw
			{
				...
				ShowPlayerDialog(playerid, DIALOG_ATMWITHDRAW, DIALOG_STYLE_LIST, mstr, "Rp.50,000\nRp.1.000.000\nRp.2.000.000\nRp.5.000.000\nRp.10.000.000", "Withdraw", "Cancel");
			}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					Info(playerid, "ATM withdraw "LG_E"Rp.50,000");
				}
				case 1:
				{
					...
					pData[playerid][pBankMoney] -= 1000000;
					Info(playerid, "ATM withdraw "LG_E"Rp.1.000.000");
				}
				case 2:
				{
					if(pData[playerid][pBankMoney] < 2000000)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 2000000);
					pData[playerid][pBankMoney] -= 2000000;
					Info(playerid, "ATM withdraw "LG_E"Rp.2.000.000");
				}
				case 3:
				{
					if(pData[playerid][pBankMoney] < 5000000)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 5000000);
					pData[playerid][pBankMoney] -= 5000000;
					Info(playerid, "ATM withdraw "LG_E"Rp.5.000.000");
				}
				case 4:
				{
					if(pData[playerid][pBankMoney] < 10000000)
						return Error(playerid, "Not enough balance!");
					
					GivePlayerMoneyEx(playerid, 10000000);
					pData[playerid][pBankMoney] -= 10000000;
					Info(playerid, "ATM withdraw "LG_E"Rp.10.000.000");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"AsiaPride Roleplay: "LB_E"Bank", lstr, "Close", "");
		}
	}
	if(dialogid == DIALOG_BANKWITHDRAW)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Revolium RP: "LB_E"Bank", lstr, "Close", "");
		}
	}
	...
	if(dialogid == DIALOG_BUYPV)
	{
		...
		if(response)
		{
			...
			Float:x = 654.116577, Float:y = -1541.230346, Float:z = 15.102634, Float:a = 654.116577, price = GetVehicleCost(GetVehicleModel(GetPlayerVehicleID(playerid)));
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					//Bikes
					new str[1024];
					/*format(str, sizeof(str), "%s"WHITE_E"%s\t"LG_E"%s\n", str, GetVehicleModelName(471), FormatMoney(GetVehicleCost(471)));
					...
					GetVehicleModelName(471), FormatMoney(GetVehicleCost(471)), 
				...
				case 1:
				{
					...
					new str[1024];
					format(str, sizeof(str), "Kendaraan\tHarga\n"WHITE_E"%s\t\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n%s\t"LG_E"%s\n", 
					GetVehicleModelName(527), FormatMoney(GetVehicleCost(527)), 
					GetVehicleModelName(589), FormatMoney(GetVehicleCost(589)),
					GetVehicleModelName(526), FormatMoney(GetVehicleCost(526)),
				...
				case 2:
				{
					...
					GetVehicleModelName(533), FormatMoney(GetVehicleCost(533)),
					...
					GetVehicleModelName(418), FormatMoney(GetVehicleCost(418)),
	...
	if(dialogid == DIALOG_RENT_JOBCARS)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					callcmd::jualsusutobrut(playerid, "");
				}
				case 1:
				{
					callcmd::pofcdbxvcfw(playerid, "");
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa sepeda "PINK_E"%s "WHITE_E"dengan harga "LG_E"Rp.20.000", GetVehicleModelName(modelid));
				...
				case 1:
				{
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa motor "PINK_E"%s "WHITE_E"dengan harga "LG_E"Rp.50.000", GetVehicleModelName(modelid));
					ShowPlayerDialog(playerid, DIALOG_RENT_BIKECONFIRM, DIALOG_STYLE_MSGBOX, "Rental Bike", tstr, "Rental", "Batal");
				}
				case 2:
				{
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa mobil "PINK_E"%s "WHITE_E"dengan harga "LG_E"Rp.125.000", GetVehicleModelName(modelid));
					ShowPlayerDialog(playerid, DIALOG_RENT_BIKECONFIRM, DIALOG_STYLE_MSGBOX, "Rental Bike", tstr, "Rental", "Batal");
				}
				case 3:
				{
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa mobil "PINK_E"%s "WHITE_E"dengan harga "LG_E"Rp.225.000", GetVehicleModelName(modelid));
					ShowPlayerDialog(playerid, DIALOG_RENT_BIKECONFIRM, DIALOG_STYLE_MSGBOX, "Rental Bike", tstr, "Rental", "Batal");
				}
			}
		}
	...
	if(dialogid == DIALOG_RENT_BIKECONFIRM)
	{
		...
		if(response)
		{
			...
			GivePlayerMoneyEx(playerid, -cost);
			ShowItemBox(playerid, "Uang", "Removed_-cost", 1212, 2);
			new cQuery[1024];
			new Float:x,Float:y,Float:z, Float:a;
	...
	if(dialogid == DIALOG_BUS)
	{
		...
		if(response)
		{
			ShowPlayerDialog(playerid, DIALOG_RUTE_BUS, DIALOG_STYLE_LIST, "Pilih Rute Bus", ">> Arah Singapure", "Pilih", "Batal");
		}
		else
		{
			RemovePlayerFromVehicle(playerid);
	...
	if(dialogid == WS_WITHDRAWMONEY)
	{
		if(response)
		{
			...
			if(amount < 1)
				return Error(playerid, "Minimum amount is Rp.1000000");
			...
			switch(listitem)
			{
				...
				case 4:
				{
					...
					if(VendingData[vid][vendingMoney] < 1000)
						return Error(playerid, "Setidaknya anda mempunyai uang dalamam vending anda senilai Rp.100000 untuk merestock product.");
	...
	if(dialogid == DIALOG_BUY_TOL)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0://10 Tiket (Rp.10,000)
				{
					if(pData[playerid][pMoney] < 10000)
					{
						Error(playerid, "Uang kamu tidak cukup untuk membeli Tiket.");
						return 1;
					}
					GivePlayerMoneyEx(playerid, -10000);
					pData[playerid][pTikettol] += 10;
					Info(playerid, "Kamu sukses membeli 10 Tiket TOL seharga Rp.10,000");
				}
				case 1://20 Tiket (Rp.20,000)
				{
					...
					pData[playerid][pTikettol] += 20;
					Info(playerid, "Kamu sukses membeli 20 Tiket TOL seharga Rp.20,000");
				}
				case 2://50 Tiket (Rp.50,000)
				{
					...
					GivePlayerMoneyEx(playerid, -50000);
					pData[playerid][pTikettol] += 50;
					Info(playerid, "Kamu sukses membeli 50 Tiket TOL seharga Rp.50,000");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_SPAWN_1)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 1://desa
				{
					SetPlayerPos(playerid, 2270.36, 29.62, 26.57);
					SetPlayerFacingAngle(playerid, 90.8201);
					SetPlayerInterior(playerid, 0);
					SetPlayerVirtualWorld(playerid, 0);	
					ClearAnimations(playerid);		
					Info(playerid, "Kamu spawn di DESA.");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_VERIFYCODE)
	{
		if(response)
		{
			...
			if(strval(inputtext) == pData[playerid][pVerifyCode])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration AsiaPride Roleplay", lstring, "Register", "Quit");
				return 1;
			}
	...
	if(dialogid == DIALOG_PHONE_CONTACT)
	{
		if (response)
		{
		    if (listitem)
			{
                ...
                //Error(playerid, "Kamu harus memilih kontak yang sudah terdaftar.");
	...
	if(dialogid == DIALOG_BUYPARTS_DONE)
	{
		...
		switch(listitem)
		{
			case 0:
			{
				...
				pvData[vehid][cPosX] = 1320.603515;
				pvData[vehid][cPosY] = -865.152038;
				pvData[vehid][cPosZ] = 39.478111;
				pvData[vehid][cPosA] = 2.5077;
				SetValidVehicleHealth(pvData[vehid][cVeh], 1500);
				SetVehiclePos(pvData[vehid][cVeh], 1320.603515, -865.152038, 39.478111);
	...
	if(dialogid == DIALOG_PAYTAX_HOUSE)
	{
		if(response)
		{
			...
			if(GetPlayerMoney(playerid) < 5000000)
				return Error(playerid, "Kamu harus memiliki uang sejumlah Rp5.000.000"GREEN_E" "WHITE_E"untuk membayar pajak asset rumahnmu");

			hData[id][hVisit] = gettime() + (86400 * 30);
			House_Refresh(id);
			House_Save(id);
			GivePlayerMoneyEx(playerid, -5000000);
			Info(playerid, "Kamu telah membayar pajak rumah (%s (ID: %d)) dengan harga "GREEN_E"Rp.5.000.000 "WHITE_E"(/paytax untuk mengeceknya)", GetLocation(hData[id][hExtposX], hData[id][hExtposY], hData[id][hExtposZ]), id);
		}
	}
	...
	if(dialogid == DIALOG_SERVERMONEY_STORAGE)
	{
		...
		else 
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Revolium City Money", lstr, "Manage", "Close");
		}
	}
	...
	if(dialogid == DIALOG_AIRDROP)
	{
		if (response)
		{
            ...
            //Info(playerid, "Anda telah berhasil meminta persetujuan untuk meminta kontak dari %s.", pData[targetp2][pName]);
            Info(playerid, "Fitur ini dalam masa perkembangan... ( sementara pakai Tambah Kontak )");
			...
			ShowPlayerDialog(targetp2, DIALOG_AIRDROP1, DIALOG_STYLE_MSGBOX, "AsiaPride - Airdrop", gstr, "Iya", "Tidak");
			return 1;
		}
	}
          ...
          /*for (new i = 0; i != MAX_CONTACTS; i ++)
			{
				if (!ContactData[targetp2][i][contactExists])
				{
	            	...
	            	ContactData[targetp2][i][contactNumber] = strval(inputtext);

					format(ContactData[targetp2][i][contactName], 32, pData[playerid][pName]);

					mysql_format(g_SQL, string, sizeof(string), "INSERT INTO `contacts` (`ID`, `contactName`, `contactNumber`) VALUES('%d', '%s', '%d')", pData[targetp2][pID], pData[targetp2][pName], ContactData[targetp2][i][contactNumber]);
					mysql_tquery(g_SQL, string, "OnContactAdd", "dd", targetp2, i);
					Info(targetp2, "You have added \"%s\" to your contacts.", pData[playerid][pName]);
	                return 1;
				}
		   */
		}
