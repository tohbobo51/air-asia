...
stock ShowContacts(playerid)
{
	...
	string = ""YELLOW_E"Daftar Kontak\n";

	for (new i = 0; i != MAX_CONTACTS; i ++) if (ContactData[playerid][i][contactExists])
	{
	    format(string, sizeof(string), "%s - No.%d\n", ContactData[playerid][i][contactName], ContactData[playerid][i][contactNumber]);
