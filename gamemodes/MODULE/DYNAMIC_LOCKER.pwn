...
Locker_Refresh(lid)
{
    if(lid != -1)
    {
		...
		lData[lid][lPickup] = CreateDynamicPickup(1275, 23, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]+0.2, 0, lData[lid][lInt], _, 4);
...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return Usage(playerid, "/createlocker [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.PEDAGANG 6.GOJEK 7.MEKANIK]");
	
	if(type < 1 || type > 7) return Error(playerid, "Invalid type.");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return Usage(playerid, "/editlocker [id] [type] [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.PEDAGANG 6.GOJEK 7.MEKANIK]");
