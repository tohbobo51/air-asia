...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name(0, "menikah", name);
	format(pData[playerid][pMenikah], 40, "%s", name);
	...
	cache_get_value_name_int(0, "boosttime", pData[playerid][pBoostTime]);
	...
	cache_get_value_name_int(0, "characterstory", pData[playerid][pCharacterStory]);
	...
	cache_get_value_name_int(0, "tikettol", pData[playerid][pTikettol]);
	...
	cache_get_value_name_int(0, "divisi", pData[playerid][pDivisi]);
	...
	cache_get_value_name_int(0, "batu", pData[playerid][Batu]);
	cache_get_value_name_int(0, "batucuci", pData[playerid][BatuCucian]);
	cache_get_value_name_int(0, "emas", pData[playerid][bEmas]);
	cache_get_value_name_int(0, "tembaga", pData[playerid][bTembaga]);
	cache_get_value_name_int(0, "alumunium", pData[playerid][bAlumunium]);
	...
	cache_get_value_name_int(0, "nasi", pData[playerid][pNasi]);
	cache_get_value_name_int(0, "burger", pData[playerid][pBurger]);
	cache_get_value_name_int(0, "aGoreng", pData[playerid][pAGoreng]);
...
function OnPlayerRegister(playerid)
{
	...
	format(pData[playerid][pMenikah], MAX_PLAYER_NAME, "None");
...
function PhoneNumber(playerid, phone)
{
	...
	{
		//Rekening Exist
		new query[128], rand = RandomEx(1111111, 9999999);
...
function LoadStats(playerid, PlayersName[])
{
	...
	else
	{
		...
		format(gstr,sizeof(gstr),"%sFaction: [%s] | Family: [%s] | Phone Status : [%s] | Nama Samaran : [{0099ff}%s{ffffff}]\n\n", gstr, fac, fname, pstatus);
...
{
	SetPlayerPositionEx(playerid, 1554.18, -1611.37, 13.38, 178.17, 2000);
...
function SpawnAwal(playerid)
{
	if(pData[playerid][pSpawnList] == 1)
	{
		SetPlayerPos(playerid, 444.9741,-1796.7011,8.0287);
		SetPlayerFacingAngle(playerid, 6.4081); //hadapan player
