CMD:acmds(playerid)
{
	...
	if(pData[playerid][pAdmin] >= 1)
	{
		...
		strcat(line3, "{ff0000}[Volunteer]: {ffffff}/a, /aduty, /asay, /arevive, /setcs, /sendto, /spec, /getip, /aka, /akaip, /vmodels, /vehname\n");
		strcat(line3, "{ff0000}[Volunteer]: {ffffff}/respawnsapd, /respawnsags, /respawnsamd, /respawnsana, /respawndmv, /respawnjobs\n\n");
	}
	if(pData[playerid][pAdmin] >= 2)
	{
		strcat(line3, "{ff0000}[Helper]: {ffffff}/adminjail, /togooc /goto, /freeze, /unfreeze, /slap, /caps, /aitems, /astats, /adelays\n");
	...
	{
		strcat(line3, "{ff0000}[Administrator]: {ffffff}/makequiz, /arelease, /settime, /setweather, /gotoco, /setlevel, /setname, /setvip, /giveweap\n");
	...
	if(pData[playerid][pAdmin] >= 5)
	{
		...
		strcat(line3, "{ff0000}[Management]: {ffffff}/setprice, /anticheat, /setstat, /setbooster, /createactor, /editactor , /deleteactor, /gotoactor\n\n");
	}
	...
	strcat(line3, "\n"BLUE_E"KP:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Napze:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:setcs(playerid, params[])
{
	...
	Servers(playerid, "Kamu telah mengset Character Story "X1_BLUE"%s "YELLOW_E"menjadi = approved %s", ReturnName(targetid), option);
	Servers(targetid, "Admin "X1_BLUE"%s "YELLOW_E"telah mengset Character Story kamu menjadi menjadi = approved ", ReturnName(playerid), option);
...
CMD:makequiz(playerid, params[])
{
	if(pData[playerid][pAdmin] < 5)
	{
		...
		if(!strcmp(tmp, "question", true, 8))
		{
			...
			format(string, sizeof(string), "{7fffd4}[QUIZ]: {ffff00}%s?, Hadiah {00FF00}Rp%d.", str, qprs);
			SendClientMessageToAll(0xFFFF00FF, string);
			SendClientMessageToAll(-1,"{ffff00}Anda bisa memberi jawaban dengan menggunakan /jawab.");
			quiz = 1;
		}
...
CMD:jawab(playerid, params[])
{
	new tmp[256], string[256];
	if (quiz == 0) return Error(playerid, "Sayangnya tidak ada kuis dari admin server.");
	if (sscanf(params, "s[256]", tmp)) return Usage(playerid, "/jawab [jawaban]");
...
CMD:hcmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"KP:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"KP:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:ahide(playerid, params[])
{
	...
	pData[playerid][pAhide] = (!pData[playerid][pAhide]);
	SendClientMessageEx(playerid, COLOR_LRED, "INFO: {FFFFFF}You are now %s {FFFFFF}%s the admin list.", (pData[playerid][pAhide]) ? ("Hidden") : ("Visible"), (pData[playerid][pAhide]) ? ("from") : ("in"));
	return 1;
}
...
CMD:aduty(playerid, params[])
{
	...
	if(!pData[playerid][pAdminDuty])
    {
		if(pData[playerid][pAdmin] > 0)
		{
			if(pData[playerid][pGender] == 1)
			{
				...
				SendClientMessageToAllEx(COLOR_YELLOW, "{FF0000}%s {FFFFFF}TELAH ON DUTY ADMIN GUNAKAN {00FFFF}[/ASK]{FFFFFF}UNTUK MEMINTA BANTUAN.", pData[playerid][pName]);
			}
			else
			{
				SetPlayerColor(playerid, 0xFF000000);
				pData[playerid][pAdminDuty] = 1;
				SetPlayerName(playerid, pData[playerid][pAdminname]);
				SendClientMessageToAllEx(COLOR_YELLOW, "{FFFFFF}%s {FFFFFF}TELAH ON DUTY ADMIN GUNAKAN {00FFFF}[/ASK]{FFFFFF}UNTUK MEMINTA BANTUAN.", pData[playerid][pName]);
			}
		}
		else
		{
			pData[playerid][pAdminDuty] = 1;
			SetPlayerName(playerid, pData[playerid][pAdminname]);
			SendClientMessageToAllEx(COLOR_YELLOW, "{FFFFFF}%s telah on helper duty.", pData[playerid][pName]);
		}
		new dc[128];
	    format(dc, sizeof(dc),  "```\n<!> %s telah onduty admin dengan nama %s```", pData[playerid][pName], pData[playerid][pAdminname]);
    ...
    else
    {
        ...
        SendClientMessageToAllEx(COLOR_RED, "{FFFFFF} %s TELAH OFF DUTY ADMIN.", pData[playerid][pName]);
		new dc[128];
	    format(dc, sizeof(dc),  "```\n<!> %s telah off duty admin dan nama berubah menjadi normal dengan nama %s```", pData[playerid][pAdminname], pData[playerid][pName]);
	    SendDiscordMessage(13, dc);
    }
	return 1;
}
...
{
	if(pData[playerid][pAdmin] < 6) return Error(playerid,"Harus Admin level 6.");
	new otherid;
    if(sscanf(params, "u", otherid))
...
CMD:sendto(playerid, params[])
{
    ...
    {
        Usage(playerid, "/sendto [player] [name]");
	...
	if(!strcmp(type,"ls")) 
	{
		...
		{
            SetVehiclePos(GetPlayerVehicleID(otherid),444.974182, -1796.701171, 8.028781);
        }
        else 
		{
            SetPlayerPosition(otherid, 444.974182,-1796.701171,8.028781);
        }
    ...
    else if(!strcmp(type,"sf")) 
	{
		...
		{
            SetPlayerPosition(otherid, 2040.798583, 1332.868164, 1002.450012);
        }
        SetPlayerFacingAngle(otherid,179.4088);
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
    ...
    else if(!strcmp(type,"lv")) 
	{
		...
		{
            SetPlayerPosition(otherid, 785.0345, -1334.3840, 716.4608);
        }
        SetPlayerFacingAngle(otherid,179.4088);
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW, 5.PEDAGANG, 6.GOJEK, 7.MEKANIK] [rank 1-13]");
...
CMD:setleader(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid, fid))
        return Usage(playerid, "/setleader [playerid/PartOfName] [0.None, 1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW, 5.Pedagang, 6.GOJEK, 7.MEKANKK]");
...
CMD:sethpall(playerid, params[])
{
	...
	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah membagikan darah ke semua player yang ada di kota", pData[playerid][pAdminname]);
...
CMD:setprice(playerid, params[])
{
	...
	if(sscanf(params, "s[64]S()[128]", name, string))
    {
		...
		SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} [FoodPrice], [FishPrice], [GsPrice] [AyamFillPrice]");
        return 1;
    }
	if(!strcmp(name, "materialprice", true))
    {
        ...
        if(price < 0 || price > 100000)
            return Error(playerid, "Kamu harus menspesifi setidaknya dari 0 sampai 100.000.");
    ...
    else if(!strcmp(name, "ayamfillprice", true))
    {
		new price;
        if(sscanf(string, "d", price))
        ...
        AyamFillPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set ayam price to %s.", pData[playerid][pAdminname], FormatMoney(price));
    }
	return 1;
}
...
CMD:setstock(playerid, params[])
{
	...
	if(sscanf(params, "s[64]S()[128]", name, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} [material], [component], [product], [gasoil] [food] [obat] [pedagang]");
        return 1;
    }
    ...
    else if(!strcmp(name, "pedagang", true))
    {
        ...
        if(stok < 0 || stok > 100000)
            return Error(playerid, "You must specify at least 0 or 100.000.");
...
CMD:setstat(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]S()[32]", otherid, name, param))
		{
			...
			Info(playerid, "Names: Level, LevelUp, Gold, Money, Bank, Health, Armour, Hunger, Energy, Stress");
		...
		{			
			...
			mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET stress = %i WHERE reg_id = %i", pData[otherid][pStress], pData[otherid][pID]);
	    	mysql_tquery(g_SQL, query);
		}
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FF0000}Moderator Commands | Alter Pride Roleplay", string, "Close","");
	return 1;
}
