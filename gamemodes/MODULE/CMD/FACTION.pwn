...
CMD:r(playerid, params[])
{
    ...
    if(pData[playerid][pFaction] == 1) {
        SendFactionMessage(1, COLOR_RADIO, "** [SAPD Radio] %s(%s) %s: %s", GetFactionRank(playerid), GetFactionDivisi(playerid), pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_RADIO, 10.0, 3000);
    }
    else if(pData[playerid][pFaction] == 2) {
...
//	if(pData[playerid][pSuspectTimer] > 1)
	...
	Info(playerid, "Lokasi %s ditemukan, segera pergi kesana!", ReturnName(otherid));
	Info(otherid, "Dokter %s telah menerima laporanmu, silahkan tunggu!", ReturnName(playerid));
...
CMD:gov(playerid, params[])
{
	...
	if(pData[playerid][pFaction] == 1)
	{
		ShowPlayerDialog(playerid, DIALOG_HP_TWITTER, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	else if(pData[playerid][pFaction] == 2)
	{
		ShowPlayerDialog(playerid, DIALOG_SKIBIDI, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	else if(pData[playerid][pFaction] == 3)
	{
		ShowPlayerDialog(playerid, DIALOG_HP_ABANGKU, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	else if(pData[playerid][pFaction] == 4)
	{
		ShowPlayerDialog(playerid, DIALOG_RORMANG, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	else if(pData[playerid][pFaction] == 5)
	{
		ShowPlayerDialog(playerid, DIALOG_ROMAKEPALA, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	else if(pData[playerid][pFaction] == 6)
	{
		ShowPlayerDialog(playerid, DIALOG_AURORA, DIALOG_STYLE_INPUT, "Pesan Umum", "Masukan Text  Yang Mau Di Post", "Kirim", "Batal");
	}
	return 1;
}
...
{
	new rank, otherid;
	if(pData[playerid][pFactionLead] == 8 && pData[playerid][pFactionRank] > 12)
...
CMD:locker(playerid, params[])
{
	...
	foreach(new lid : Lockers)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]))
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_GOJEK, DIALOG_STYLE_LIST, "GOJEK Locker", "Toggle Duty", "Proceed", "Cancel");
			}
...
CMD:setplate(playerid, params[])
{
	...
	new vehid;
	if(sscanf(params, "d", vehid)) return Usage(playerid, "/setplate [vehid]");
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				...
				format(pvData[i][cPlate], 32, "GL %d RP", rand);
				SetVehicleNumberPlate(pvData[i][cVeh], pvData[i][cPlate]);
				pvData[i][cPlateTime] = gettime() + (30 * 86400);
				Info(playerid, "Model: %s || New plate: %s || Plate Time: %s", GetVehicleModelName(pvData[i][cModel]), pvData[i][cPlate], ReturnTimelapse(gettime(), pvData[i][cPlateTime]));
			}
...
CMD:setsim(playerid, params[])
{
    ...
    if(sscanf(params, "u", to_player))
        return Usage(playerid, "/setsim [playerid/PartOfName]");
	...
	format(lstr, sizeof(lstr), "Surat Izin Mengemudi %s", pData[to_player][pName]);
	...
	Info(to_player, "Anda mendapatkan surat	Sim dari kantor kepolisian");
	SendNearbyMessage(playerid, 30.0, COLOR_BLUE, "** %s Memberikan Surat Izin Mengemudi Kepada %s", ReturnName(playerid), ReturnName(to_player));
	pData[to_player][pDriveLic] = 1;
	pData[to_player][pDriveLicTime] = gettime() +  (30 * 86400);
	Info(playerid, "Anda Telah Memberikan Surat Izin Mengemudi kepada %s", ReturnName(to_player));
	return 1;
}
...
CMD:setweaplic(playerid, params[])
{
			if(pData[playerid][pFaction] != 1)
        return Error(playerid, "Kamu bukan anggota Sapd.");
	new to_player;
    if(sscanf(params, "u", to_player))
        return Usage(playerid, "/setweaplic [playerid/PartOfName]");
	...
	new sext[40], lstr[128], mstr[128];	
	if(pData[to_player][pGender] == 1) { sext = "Laki-Laki"; } else { sext = "Perempuan"; }
	format(lstr, sizeof(lstr), "Licenses Weapon %s", pData[to_player][pName]);
	format(mstr,sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 30 hari!", pData[to_player][pName], pData[to_player][pAge], sext);
	ShowPlayerDialog(to_player, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, lstr, mstr, "Tutup", "");
	Info(to_player, "Anda mendapatkan surat	Licenses Weapon dari Kantor Kepolisan ");
	SendNearbyMessage(playerid, 30.0, COLOR_BLUE, "** %s Memberikan Surat Licenses Weapon Kepada %s", ReturnName(playerid), ReturnName(to_player));
	pData[to_player][pWeaponLic] = 1;
	pData[to_player][pWeaponLicTime] = gettime() +  (30 * 86400);
	Info(playerid, "Anda Telah Memberikan Surat Licenses Weapon kepada %s", ReturnName(to_player));
	return 1;
}
...
CMD:release(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1960.78, 1393.53, 2889.81))
...
CMD:arrest(playerid, params[])
{
	...
	if(denda < 100 || denda > 200000)
        return Error(playerid, "The specified denda can't be below 100 or above 200,000.");
...
CMD:setktp(playerid, params[])
{
	...
	new to_player;
    if(sscanf(params, "u", to_player))
        return Usage(playerid, "/setktp [playerid/PartOfName]");
	...
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 15 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
	...
	Info(playerid, "Anda Telah Memberikan Ktp kepada %s", ReturnName(to_player));
	
	return 1;
}

CMD:setbizlic(playerid, params[])
{
	...
	new to_player;
    if(sscanf(params, "u", to_player))
        return Usage(playerid, "/setbizlic [playerid/PartOfName]");
	...
	SendNearbyMessage(playerid, 30.0, COLOR_BLUE, "** %s Memberikan Surat Licenses Business Kepada %s", ReturnName(playerid), ReturnName(to_player));
...
CMD:checkcitymoney(playerid, params)
{
	...
	ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Asia Pride City Money", lstr, "Manage", "Close");
	return 1;
}
...
CMD:lspddiv(playerid, params[])// TEU HSIU ASD MRU GCLU PTRU SWAT DETECTIVE AI
{
	new alevel, otherid, tmp[64];
	if(pData[playerid][pFactionLead] != 1) return Error(playerid, "Kamu bukan leader SAPD");
	if(sscanf(params, "ud", otherid, alevel))
	{
	    Usage(playerid, "/lspddiv <ID/Name> < 1 = TEU, 2 = HSIU, 3 = ASD, 4 = MRU, 5 = GLCU, 6 = PTRU, 7 = SWAT, 8 = DETECTIVE, 9 = AI>");
	    return true;
	}
	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Player not connected!");
	...
	if(alevel < 1)
		return Error(playerid, "Level can't be lower than 1!");
	...
	pData[otherid][pDivisi] = alevel;
	Servers(playerid, "You has set faction division %s(%d) to level %d", pData[otherid][pName], otherid, alevel);
	Servers(otherid, "%s(%d) has set faction division level to %d", pData[otherid][pName], playerid, alevel);
...
CMD:revive(playerid, params[])
{
    ...
    if(pData[playerid][pMedicine] < 1)
    	return Error(playerid, "Tidak dapat Revive karena anda tidak memiliki Medicine");
...
forward TreatmentPlayer(playerid, p2);
public TreatmentPlayer(playerid, p2)
{
	if(!IsPlayerConnected(playerid)) return 0;
	if(pData[playerid][pProductingStatus] != 1) return 0;
	...
	if(pData[playerid][pActivityTime] >= 100)
	{
		...
		SetPlayerHealthEx(p2, 100);
		InfoTD_MSG(playerid, 8000, "Treatment Sucess!");
		InfoTD_MSG(p2, 8000, "Treatment Sucess!");
		KillTimer(pData[playerid][pProducting]);
		pData[playerid][pProductingStatus] = 0;
		pData[playerid][pActivityTime] = 0;
		HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5, 1229.49, -1427.36, 13.55)) return Error(playerid, "Kamu harus di dapur!");
	if(pData[playerid][pLevel] < 2) return Error(playerid, "Kamu tidak memiliki skill masak.");
	if(pData[playerid][pFaction] != 5)
        return Error(playerid, "You must be part of a Pedagang.");

	new Dstring[512];
	format(Dstring, sizeof(Dstring), "Jenis (Jumlah)\tBahan\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sKebab (5)\tAyam fillet 2 + snack 2\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sNasi Bungkus(5)\tPotato 2 + Ayam Fillet 3\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sUltra Milk(5)\tPotato 2 + Sprunk 2\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sBurger(5)\tAyamFillet 3 + Potato 3\n", Dstring);
	format(Dstring, sizeof(Dstring), "{ffffff}%sIkan Goreng \t20 Lbs\n", Dstring);
	//format(Dstring, sizeof(Dstring), "{ffffff}%sAK-47(ammo 100)\t500\n", Dstring);
	//format(Dstring, sizeof(Dstring), "{ffffff}%sLockPick( 1 )\t300\n", Dstring);
	ShowPlayerDialog(playerid, DIALOG_MENUMASAK, DIALOG_STYLE_TABLIST_HEADERS, "Masak", Dstring, "Mulai", "Cancel");
	return 1;
}

function Menumasak(playerid, gunid, ammo)
{
	if(!IsPlayerConnected(playerid)) return 0;
	...
	if(pData[playerid][pActivityTime] >= 100)
	{
		...
		Info(playerid, "Makanan sudah matang.");
		TogglePlayerControllable(playerid, 1);
		InfoTD_MSG(playerid, 8000, "Makanan Masak!");
...
{
    if(IsPlayerInRangeOfPoint(playerid,2.0,1228.08, -1424.94, 13.50))
    {
        ShowPlayerDialog(playerid, DIALOG_MENU, DIALOG_STYLE_LIST, "MENU PEDAGANG","BURGER\t"GREEN_E"\nULTRA MILK\t"GREEN_E"$\nKEBAB\t"GREEN_E"\nNASI BUNGKUS","PILIH","CLOSE");
	}
	return 1;
}
